...
__host__ __device__ float compute_max_step_physics(float E, const RLUT& lut) {
    ...
    // Energy-dependent refinement near Bragg peak
    if (E < 10.0f) {
        delta_R_max = fminf(delta_R_max, 0.2f);  // Near Bragg: 0.2mm max
    } else if (E < 50.0f) {
        delta_R_max = fminf(delta_R_max, 0.5f);  // Mid-range: 0.5mm max
    }

    // Absolute maximum
    delta_R_max = fminf(delta_R_max, 1.0f);  // Never > 1mm
