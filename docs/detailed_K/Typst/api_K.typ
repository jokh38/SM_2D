...
이 문서는 모듈별로 구성된 SM_2D의 모든 공개 API에 대한 포괄적인 참조를 제공합니다.
...
```cpp
// 0.1에서 250 MeV까지 로그 간격 에너지 그리드 생성
...
```cpp
struct AngularGrid {
    const int N_theta;          // 세타 빈 수
    const float theta_min;      // 최소 각도 [도]
    const float theta_max;      // 최대 각도 [도]
...
struct RLUT {
    ...
    std::vector<float> log_E;    // 미리 계산된 log(E)
    std::vector<float> log_R;    // 미리 계산된 log(R)
    std::vector<float> log_S;    // 미리 계산된 log(S)

    // 에너지에서 사거리(로그-로그 보간)
    float lookup_R(float E_MeV) const;

    // 에너지에서 stopping power
...
```cpp
struct BoundaryLoss {
    float weight[4];   // 면당 손실된 가중치
    double energy[4];  // 면당 손실된 에너지
...
struct LateralSpreadResult {
    float z_mm;               // 측정 깊이
    float sigma_sim;          // 시뮬레이션된 횡방향 시그마
...
struct SimulationResult {
    int exit_code;                      // 0 = 성공
    int n_steps_completed;              // 실행된 단계
    double wall_time_seconds;           // 런타임

    GlobalAudit audit;                  // 보존 감사
    BraggPeakResult bragg_peak;         // 브래그 피크 분석

    std::string dose_file;              // 출력 파일
    std::string pdd_file;
