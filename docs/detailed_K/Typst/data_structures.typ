...
Compact 24-bit encoding of $(theta, E)$ phase-space coordinates for efficient GPU storage.
...
#figure(
  table(
    ...
    inset: 8pt,
    align: left,
    table.header([*Dimension*], [*Bins*], [*Description*]),
    [theta_."local"], [8], [Local angular subdivision],
    [E_."local"], [4], [Local energy subdivision],
    [x_"sub"], [4], [Transverse position within cell],
    [z_"sub"], [4], [Depth position within cell],
  ),
  caption: [Local Bin Dimensions],
)

#text(size: 10pt)[*Total: 8 × 4 × 4 × 4 = 512 local bins per block*]
...
#figure(
  table(
    ...
    table.header([*x_"sub"*], [*Offset (times dif x)*]),
    [0], [-0.375],
    [1], [-0.125],
    [2], [+0.125],
    [3], [+0.375],
  ),
  caption: [Sub-bin Center Offsets],
)

== 5. Phase-Space Storage (PsiC)
...
* Memory per slot: 512 bins × 4 bytes = 2 KB
* Total storage: ~8 GB (fully dense)
* Actual usage: ~1.1 GB (sparse)
...
#figure(
  table(
    ...
    inset: 8pt,
    align: left,
    table.header([*Face*], [*Direction*], [*Index*]),
    [+z], [Forward], [0],
    [-z], [Backward], [1],
    [+x], [Right], [2],
    [-x], [Left], [3],
  ),
  caption: [Face Index Convention],
)
...
#figure(
  table(
    ...
    inset: 8pt,
    align: left,
    table.header([*Component*], [*Size per Instance*], [*Total Size*]),
    [EnergyGrid], [~4 KB], [4 KB],
    [AngularGrid], [~8 KB], [8 KB],
    [PsiC], [~1.1 GB], [1.1 GB],
    [OutflowBuckets], [~32 KB per cell], [~4 GB (all cells)],
  ),
  caption: [Memory Summary],
)

#text(size: 10pt)[*Active Working Set: ~2.2 GB*]

== Design Rationale

=== 1. Why Block-Sparse?

* Phase space is mostly empty (particles occupy limited $(theta, E)$ regions)
...
* O(K_b) search is fast (K_b = 32 is small)
* Avoids dynamic allocation on GPU

---
#set align(center)
*SM_2D Data Structures Documentation*
